...
try {
    ...
    if ($Init -or $Upgrade) {
        if (!$NoBackend) {
            ...
            $tfbackendArgs = ""
            $env:TF_STATE_backend_storage_account   ??= $env:TF_STATE_BACKEND_STORAGE_ACCOUNT
            $env:TF_STATE_backend_storage_container ??= $env:TF_STATE_BACKEND_STORAGE_CONTAINER
            $env:TF_STATE_backend_resource_group    ??= $env:TF_STATE_BACKEND_RESOURCE_GROUP
            if ($newBackend) {
